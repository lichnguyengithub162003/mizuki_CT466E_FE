...
import { useAdminRefundCounts } from '@/queries/admin'

defineProps<{ compact?: boolean }>()

const countsQuery = useAdminRefundCounts()
const pending = computed(() => countsQuery.data.value?.pending_action ?? 0)
...
<template>
  <span
    v-if="pending > 0"
    data-testid="admin-refunds-pending-badge"
    :aria-label="`${pending} yêu cầu hoàn tiền cần xử lý`"
    :class="compact
