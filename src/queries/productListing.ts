  ...
  return useQuery(
    {
      ...
      queryFn: async () =>
        adaptProductDetail(await getProductDetail(normalizedSlug.value)),
      enabled: computed(
        () => toValue(enabled) && normalizedSlug.value.length > 0,
      ),
    },
    productListingQueryClient,
  );
...
export function useProductReviewsQuery(
  ...
  const apiRequest = computed<ProductReviewsRequest>(() => ({
    ...(normalizedRequest.value.rating !== null
      ? { rating: normalizedRequest.value.rating }
      : {}),
    ...(normalizedRequest.value.hasImages ? { has_images: true } : {}),
    ...(normalizedRequest.value.verifiedOnly ? { verified_only: true } : {}),
    sort: "newest",
    page: 1,
    per_page: normalizedRequest.value.perPage,
  }));
  ...
  return useQuery(
    {
      ...
      queryFn: async () =>
        adaptProductReviews(
          await getProductReviews(normalizedSlug.value, apiRequest.value),
        ),
      placeholderData: keepPreviousData,
      enabled: computed(
        () => toValue(enabled) && normalizedSlug.value.length > 0,
  ...
  return useQuery(
    {
      ...
      queryFn: async (): Promise<RepresentativeProducts> => {
        ...
        const [categoryResults, brandResults] = await Promise.all([
          Promise.all(
            value.categoryIds.map(async (categoryId) => ({
              categoryId,
              response: await getProductListing({
                ...baseRequest,
          ...
          Promise.all(
            value.brandIds.map(async (brandId) => ({
              brandId,
              response: await getProductListing({
                ...baseRequest,
                brand_id: brandId,
        ...
        return {
          categories: new Map(
            categoryResults.map(({ categoryId, response }) => [
              categoryId,
              response.data[0]
                ? adaptProductListItem(response.data[0])
                : undefined,
            ]),
          ),
          brands: new Map(
            brandResults.map(({ brandId, response }) => [
              brandId,
              response.data[0]
                ? adaptProductListItem(response.data[0])
                : undefined,
      ...
      enabled: computed(() => {
        ...
        return (
          toValue(enabled) &&
          (value.categoryIds.length > 0 || value.brandIds.length > 0)
        );
      }),
    },
    productListingQueryClient,
