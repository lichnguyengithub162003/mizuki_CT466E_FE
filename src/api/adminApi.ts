...
export async function getAdminList<T extends AdminRecord>(
  module: AdminModule,
  params: AdminListParams,
): Promise<AdminPage<T>> {
  const response = await apiClient.get<ApiResponse<T[]>>(collections[module], {
    params: listParams(module, params),
  });
  const meta = response.data.meta?.pagination as
    | Partial<AdminPage<T>["pagination"]>
    | undefined;
  return {
...
export async function getAdminDetail<T extends AdminRecord>(
  module: AdminModule,
  id: number | string,
): Promise<T> {
...
export async function getAdminOrderCounts(): Promise<AdminOrderCounts> {
  const response = await apiClient.get<ApiResponse<AdminOrderCounts>>(
    ENDPOINTS.adminOrderCounts,
  );
  return response.data.data;
}

export async function getAdminRefundCounts(): Promise<AdminRefundCounts> {
  const response = await apiClient.get<ApiResponse<AdminRefundCounts>>(
    ENDPOINTS.adminRefundCounts,
  );
  return response.data.data;
}
...
export function createAdminRecord<T>(
  module: Extract<
    AdminModule,
    "customers" | "products" | "categories" | "brands" | "promotions" | "staff"
  >,
  payload: unknown,
): Promise<T> {
...
export function updateAdminRecord<T>(
  module: Exclude<
    AdminModule,
    "orders" | "refunds" | "appointments" | "inventory"
  >,
  id: number | string,
  payload: unknown,
): Promise<T> {
...
export function deleteAdminCatalog(
  module: Extract<AdminModule, "products" | "categories" | "brands">,
  id: number | string,
): Promise<unknown> {
  const endpoint = details[module];
...
export function runOrderAction<T>(
  id: number | string,
  action: string,
): Promise<T> {
...
export function runRefundAction<T>(
  id: number | string,
  action: string,
  payload?: unknown,
): Promise<T> {
  ...
  return mutate<T>(
    "post",
    ENDPOINTS.adminRefundAction(id, endpointAction),
    payload,
  );
}

export function runAppointmentAction<T>(
  id: number | string,
  action: string,
  payload?: unknown,
): Promise<T> {
  if (action === "walk-in")
    return mutate<T>("post", `${ENDPOINTS.adminAppointments}/walk-in`, payload);
  return mutate<T>(
    "post",
    ENDPOINTS.adminAppointmentAction(id, action.replaceAll("_", "-")),
    payload,
  );
}

export function adjustInventory<T>(
  id: number | string,
  payload: { quantity_delta: number; reason: string },
): Promise<T> {
  return mutate<T>("post", ENDPOINTS.adminInventoryAdjust(id), payload);
}

export async function getInventoryTransactions<T extends AdminRecord>(
  id: number | string,
  params: AdminListParams,
): Promise<AdminPage<T>> {
  const response = await apiClient.get<ApiResponse<T[]>>(
    ENDPOINTS.adminInventoryTransactions(id),
    { params: compactParams(params) },
  );
  const meta = response.data.meta
    ?.pagination as unknown as AdminPage<T>["pagination"];
  return {
    items: response.data.data,
    pagination: meta ?? {
      current_page: 1,
...
export async function getDashboard<T>(
  params: Pick<AdminListParams, "branch_id"> & {
    date_from?: string;
    date_to?: string;
  },
): Promise<T> {
...
export async function getPromotionUsage<T>(id: number | string): Promise<T> {
  const response = await apiClient.get<ApiResponse<T>>(
    ENDPOINTS.adminPromotionUsage(id),
  );
  return response.data.data;
}
